const WORKER_URL = "https://insurance-quote-saver.YOUR-USERNAME.workers.dev";
...
function renderPlans() {
    ...
    Object.keys(PLAN_INFO).forEach(planKey => {
        ...
        card.innerHTML = `
            <h3>${info.label}</h3>
            <p class="plan-price">$${price.toFixed(2)} / ${periodLabel}</p>
            <p class="plan-description">${info.description}</p>
        `;
...
async function handleSubmit(event) {
    ...
    if (!response.ok) {
        ...
        resultBox.textContent = `Payment successful! Your ${selectedPlan.label} policy is confirmed at $${selectedPlan.price.toFixed(2)}/year.`;
